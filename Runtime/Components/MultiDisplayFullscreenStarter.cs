		...
		void Start()
		{
			...
			for( int d = 0; d < displays.Length; d++ )
			{
				...
				if( d == 0 ){ // First is always Main display.
					rate.denominator = (uint) _mainDisplayResolutionRequests.frequency;
					display.Activate( _mainDisplayResolutionRequests.resolution.x, _mainDisplayResolutionRequests.resolution.y, rate );
					continue;
				}
				...
				rate.denominator = (uint) request.frequency;
				display.Activate( request.resolution.x, request.resolution.y, rate );
